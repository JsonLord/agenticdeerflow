...
def _format_graph_state_for_llm(graph_response: KnowledgeGraphResponse) -> str:
    ...
    if not graph_response.nodes:
        ...
        for node in graph_response.nodes:
            node_info = f"  - Node ID: {node.id}, Label: \"{node.label}\", Status: {node.status}"
    ...
    dynamic_edges = [edge for edge in graph_response.edges if edge.type == "dynamic_traversed"]
    ...
    active_nodes = [node.label for node in graph_response.nodes if node.status == "active"]
    if active_nodes:
        text_parts.append(f"\nCurrently Active Node(s): {', '.join(active_nodes)}")
    else:
        final_completed_nodes = [node.label for node in graph_response.nodes if node.status == "final_completed"]
        error_nodes = [node.label for node in graph_response.nodes if node.status == "error"]
        ...
        if final_completed_nodes:
            ...
            all_pending_or_completed = all(n.status in ["pending", "completed"] for n in graph_response.nodes)
            if all_pending_or_completed and not any(n.status == "active" for n in graph_response.nodes):
...
async def answer_graph_question(
    ...
    """
    logger.info(f"Answering graph question for thread_id='{thread_id}': \"{user_question}\"")

    try:
        # 1. Get the current graph state
        graph_state_response = serialize_langgraph_state_for_thread(graph_executable, thread_id)
...
Answer:"""
        ...
        logger.error(f"Error in answer_graph_question for thread_id {thread_id}: {e}", exc_info=True)
