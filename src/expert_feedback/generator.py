...
def _format_graph_state_for_expert_llm(full_state_values: State) -> str:
    ...
    if full_state_values.get("messages"):
        ...
        initial_query = next(
            (
                msg.get("content")
                for msg in full_state_values["messages"]
    ...
    if current_plan_obj:
        ...
        if isinstance(current_plan_obj, Plan):  # If it's the Pydantic model
            ...
            current_plan_obj, str
        ):  # If it's a string (e.g., before validation)
            ...
            plan_text += "Steps:\n" + "\n".join(
                [
                    f"  - {s.get('title', 'N/A')}"
                    for s in current_plan_obj.get("steps", [])
                ]
            )
    ...
    if observations:
        obs_text = "\n".join(
            [f"- {obs[:300]}..." for obs in observations]
    ...
    if final_report:
        context_parts.append(
            f"Current Draft Report (or Final Report if process ended):\n{final_report[:1000]}...\n"
...
def _parse_expert_llm_output(llm_output: str) -> ExpertFeedbackResponse:
    ...
    def extract_list_items(section_text: str) -> List[str]:
        ...
        items = re.findall(
            r"^\s*[-*]\s*(.+)$|^\s*[0-9]+\.\s*(.+)$", section_text, re.MULTILINE
        )
    ...
    challenging_viewpoints_text = extract_section(
        llm_output, "Challenging Viewpoints & Alternative Perspectives"
    )
    further_research_questions_text = extract_section(
        llm_output, "Further Research Questions"
    )
    ...
    if (
        not process_judgment
        and not current_beliefs
        and not challenging_viewpoints
        and not further_research_questions
        and raw_output
    ):
        logger.warning(
            "Failed to parse structured expert feedback, using raw output for judgment."
        )
...
async def generate_expert_feedback(
    ...
    try:
        ...
        if not latest_snapshot:
            logger.warning(
                f"No state found for thread_id: {thread_id}. Cannot generate expert feedback."
            )
            return ExpertFeedbackResponse(
        ...
        logger.error(
            f"Error generating expert feedback for thread_id {thread_id}: {e}",
            exc_info=True,
        )
        return ExpertFeedbackResponse(
